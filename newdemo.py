...
import argparse
import yaml
from src.new_lightning_pano_gen import PanoGenerator
...
if args.image_path is None:
    config_file = 'configs/new_pano_generation.yaml'
...
resolutionH=config['dataset']['resolutionH']
resolutionW=config['dataset']['resolutionW']
rot = config['dataset']['rot']
fov = config['dataset']['fov']
Rs=[]
Ks=[]
count = int(360/rot)
for i in range(count):
    degree = (rot*i) % 360
    K, R = get_K_R(fov, degree, 0,
                    resolutionH, resolutionW)

    Rs.append(R)
    Ks.append(K)

images=torch.zeros((1,count,resolutionH,resolutionW, 3)).cuda()
if img is not None:
    images[0,0]=img
...
for i in range(count):
    ...
    im.save(image_path)
generate_video(image_paths, res_dir, fov, args.gen_video)
